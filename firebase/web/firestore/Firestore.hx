...
extern class Firestore {
	...
	@:overload(function<T>(reference:DocumentReference<T>, on_next:(snapshot:DocumentSnapshot<T>) -> Void, ?on_error:(error:FirestoreError) -> Void,
		?on_complete:() -> Void):Unsubscribe {})
	@:overload(function<T>(query:Query<T>, on_next:(snapshot:QuerySnapshot<T>) -> Void, ?on_error:(error:FirestoreError) -> Void,
		?on_complete:() -> Void):Unsubscribe {})
	 ...
	 * @return Bool
	 */
	public static function refEqual<T>(left:EitherType<CollectionReference<T>, DocumentReference<T>>,
		right:EitherType<CollectionReference<T>, DocumentReference<T>>):Bool;
